...
Private Type Vertex3D   ' Vertex Data
  ...
  '
  colr As Long              ' Color (red) of vertex (for gradient fill)
  colg As Long              ' Color (blue) of vertex (for gradient fill)
  colb As Long              ' Color (green) of vertex (for gradient fill)
  cola As Long              ' Color (alpha) of vertex (for gradient fill)
  '
...
Private Type Polygon3D  ' Polygon Data
  ...
  style As Long             ' Style of polygon - 0=wireframe,1=solid,2=solid/transparent,3=gradient
  '
End Type
'
...
Public Sub LoadMPLGObject(filename As String, obj As Object3D)
  ...
  Do
    ...
    If Left$(Trim$(valu$), 1) <> "" And Left$(Trim$(valu$), 1) <> "#" Then
      ...
      obj.vertex(vnum&).colr = Val(Field(valu$, " ", 4))
      '
      colrstrg$ = Field(valu$, " ", 4)
      '
      colrtype& = Val(Mid$(colrstrg$, 1, 2))
      '
      obj.vertex(vnum&).colr = Val("&h" + Mid$(colrstrg$, 1, 2) + "00")
      obj.vertex(vnum&).colg = Val("&h" + Mid$(colrstrg$, 3, 2) + "00")
      obj.vertex(vnum&).colb = Val("&h" + Mid$(colrstrg$, 5, 2) + "00")
      obj.vertex(vnum&).cola = Val("&h" + Mid$(colrstrg$, 7, 2) + "00")
      '
      vnum& = vnum& + 1
      '
...
Private Sub DrawTriangle(obj As Object3D, polyg As Long, style As Long, frm As Object)
  ...
  '
  ' The following are for a gradient fill
  '
  ...
  Select Case style
    ...
    Case 3
      ...
      vert(0).x = poly(0).x
      vert(0).y = poly(0).y
      vert(0).Red = obj.vertex(obj.Polygon(polyg).v_pointer(0)).colr
      vert(0).Green = obj.vertex(obj.Polygon(polyg).v_pointer(0)).colg
      vert(0).Blue = obj.vertex(obj.Polygon(polyg).v_pointer(0)).colb
      vert(0).Alpha = obj.vertex(obj.Polygon(polyg).v_pointer(0)).cola
      '
      vert(1).x = poly(1).x
      vert(1).y = poly(1).y
      vert(1).Red = obj.vertex(obj.Polygon(polyg).v_pointer(1)).colr
      vert(1).Green = obj.vertex(obj.Polygon(polyg).v_pointer(1)).colg
      vert(1).Blue = obj.vertex(obj.Polygon(polyg).v_pointer(1)).colb
      vert(1).Alpha = obj.vertex(obj.Polygon(polyg).v_pointer(1)).cola
      '
      vert(2).x = poly(2).x
      vert(2).y = poly(2).y
      vert(2).Red = obj.vertex(obj.Polygon(polyg).v_pointer(2)).colr
      vert(2).Green = obj.vertex(obj.Polygon(polyg).v_pointer(2)).colg
      vert(2).Blue = obj.vertex(obj.Polygon(polyg).v_pointer(2)).colb
      vert(2).Alpha = obj.vertex(obj.Polygon(polyg).v_pointer(2)).cola
      '
      gTri(0).Vertex1 = 0
      gTri(0).Vertex2 = 1
      gTri(0).Vertex3 = 2
      '
      foo& = GradientFill(frm.hdc, vert(0), 4, gTri(0), 1, GRADIENT_FILL_TRIANGLE)
  End Select
  '
End Sub
