...
Private Sub DrawObject3D(obj As Object3D, cam As Camera3D)
  ...
  For pl = 0 To obj.NumPlanes
    ...
    If Not CullPlane3D(obj.PointList(obj.PlaneList(pl).PointPtr(0)), obj.PointList(obj.PlaneList(pl).PointPtr(1)), obj.PointList(obj.PlaneList(pl).PointPtr(2)), cam) Then
      '
      poly.v(0).sx = obj.PointList(obj.PlaneList(pl).PointPtr(0)).sx: poly.v(0).sy = obj.PointList(obj.PlaneList(pl).PointPtr(0)).sy: poly.v(0).u = obj.PlaneList(pl).tcoords(0).u: poly.v(0).v = obj.PlaneList(pl).tcoords(0).v
      poly.v(1).sx = obj.PointList(obj.PlaneList(pl).PointPtr(1)).sx: poly.v(1).sy = obj.PointList(obj.PlaneList(pl).PointPtr(1)).sy: poly.v(1).u = obj.PlaneList(pl).tcoords(1).u: poly.v(1).v = obj.PlaneList(pl).tcoords(1).v
      poly.v(2).sx = obj.PointList(obj.PlaneList(pl).PointPtr(2)).sx: poly.v(2).sy = obj.PointList(obj.PlaneList(pl).PointPtr(2)).sy: poly.v(2).u = obj.PlaneList(pl).tcoords(2).u: poly.v(2).v = obj.PlaneList(pl).tcoords(2).v
      '
      Call DrawAffineTriangle(poly)
      '
    End If
    '
  Next
