...
c% = 75
FOR y& = 0 TO 191
  CALL DrawLine(VARSEG(buffer2%(0)), VARPTR(buffer2%(0)), 0, y&, 319, y&, c%)
  IF INT(y& / 5) = y& / 5 THEN c% = c% + 1
NEXT y&
'
SCREEN 13
'
...
DO
  ...
  SELECT CASE key$
    CASE up$
      mapy% = mapy% - 1: IF mapy% < 0 THEN mapy% = 0
    CASE dn$
      mapy% = mapy% + 1: IF mapy% > 11 THEN mapy% = 11
    CASE lt$
      mapx% = mapx% - 1: IF mapx% < 0 THEN mapx% = 0
    CASE rt$
      mapx% = mapx% + 1: IF mapx% > 19 THEN mapx% = 19
    CASE sp$
      IF Map(mapx%, mapy%).SpriteNum = -1 THEN
        Map(mapx%, mapy%).SpriteNum = sp%
      ELSE
        Map(mapx%, mapy%).SpriteNum = -1
      END IF
    CASE esc$
      done% = 1
  END SELECT
  ...
  ' Draw Cursor
  '
  CALL DrawLine(VARSEG(buffer1%(0)), VARPTR(buffer1%(0)), mapx% * 16, mapy% * 16, mapx% * 16 + 15, mapy% * 16, 15)
  CALL DrawLine(VARSEG(buffer1%(0)), VARPTR(buffer1%(0)), mapx% * 16 + 15, mapy% * 16, mapx% * 16 + 15, mapy% * 16 + 15, 15)
  CALL DrawLine(VARSEG(buffer1%(0)), VARPTR(buffer1%(0)), mapx% * 16 + 15, mapy% * 16 + 15, mapx% * 16, mapy% * 16 + 15, 15)
  CALL DrawLine(VARSEG(buffer1%(0)), VARPTR(buffer1%(0)), mapx% * 16, mapy% * 16 + 15, mapx% * 16, mapy% * 16, 15)
  '
  ' Copy the screen
  '
...
DrawScreen:
  ...
  FOR y% = 0 TO 11
    FOR x% = 0 TO 19
      spn% = Map(x%, y%).SpriteNum
      IF spn% <> -1 THEN
        CALL BlastPut(VARSEG(buffer1%(0)), VARPTR(buffer1%(0)), VARSEG(SpriteBuffer%(0)), VARPTR(SpriteBuffer%(spn% * 130)), x% * 16, y% * 16, 0)
      END IF
    NEXT
  NEXT
  '
...
NEXT y%
'
done% = 0: x% = 0: t% = 0: dir% = 1: movedir% = 4: froffset% = 0
'
DO
  '
  delay = TIMER
  ...
  '
  CALL BlastGet(VARSEG(buffer1%(0)), VARPTR(buffer1%(0)), VARSEG(TempBuffer%(0)), VARPTR(TempBuffer%(0)), x%, 96, x% + 15, 111)
  '
  ' Put the sprite
  '
  CALL BlastPut(VARSEG(buffer1%(0)), VARPTR(buffer1%(0)), VARSEG(SpriteBuffer%(0)), VARPTR(SpriteBuffer%((froffset% + t%) * 130)), x%, 96, 0)
  '
  ' Copy the screen
  '
  CALL BlastCopy(VARSEG(buffer1%(0)), VARPTR(buffer1%(0)), &HA000, 0)
  ...
  ' Put the background
  '
  CALL BlastPut(VARSEG(buffer1%(0)), VARPTR(buffer1%(0)), VARSEG(TempBuffer%(0)), VARPTR(TempBuffer%(0)), x%, 96, 0)
  '
  ...
  IF x% <= 0 THEN x% = 0
  '
  ' Wait for a machine independent time
  '
  DO: LOOP UNTIL TIMER - delay > .01
  '
LOOP UNTIL done%
'
'*******************************************
